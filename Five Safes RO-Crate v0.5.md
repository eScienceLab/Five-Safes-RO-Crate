...
* Permalink: <https://w3id.org/5s-crate/0.5>
* Version: 0.5
* Release notes: <https://github.com/trefx/5s-crate/releases/tag/0.5.0>
...
* Example crate "Hutch": [GitHub LFS](https://github.com/trefx/5s-crate/tree/main/0.5/example-hutch) [ro-crate-metadata.json](example-hutch/data/ro-crate-metadata.json) [ro-crate-preview.html](example-hutch/data/ro-crate-preview.html) 
...
A Five Safes RO-Crate represents a unit of computational access to sensitive information which is managed in accordance with a set of principles conforming to the 5 safe framework. The aim is to enable trusted execution in a Trusted Research Environment (TRE), from an authenticated execution request, through approval and review processes to a completed execution.

**Changes in v0.5**: This version generalises the execution model beyond workflow-only to support scripts, notebooks, queries, and interactive sessions. It also introduces software authorship and provenance tracking, enhanced disclosure control documentation, richer provenance capture across the data journey, user-attribute access metadata, dataset characterisation, and stakeholder-specific metadata views. See [Migration Notes (v0.4 → v0.5)](#migration-notes-v04--v05) for upgrade guidance.
...
```json
{
  "@id": "./",
  "@type": "Dataset",
  "conformsTo": {"@id": "https://w3id.org/5s-crate/0.5"},
  "hasPart": [
  ],
  "mainEntity": {"@id": "#execution-context"},
...
{
  "@id": "https://w3id.org/5s-crate/0.5",
  "@type": "Profile",
...
The metadata file MUST reference an execution context entity as its `mainEntity`, indicating the computation to be executed. The root data entity SHOULD include an `executionType` property to classify the execution (see [Execution Type Classification](#execution-type-classification)).

**Note**: In v0.4, `mainEntity` directly referenced a [Workflow RO-Crate](https://w3id.org/workflowhub/workflow-ro-crate/1.0) `Dataset`. In v0.5, the execution context pattern generalises this to support multiple types of execution. For backward compatibility, implementations MUST continue to accept v0.4 crates where `mainEntity` directly references a Workflow RO-Crate `Dataset`, treating such references as equivalent to `executionType: "workflow"`.


#### Execution Type Classification

The root data entity SHOULD include a RECOMMENDED property `executionType` to classify the type of execution requested:

| Value | Description |
|-------|-------------|
| `workflow` | Workflow RO-Crate execution (CWL, Nextflow, etc.) |
| `script` | Single script or code file execution |
| `notebook` | Interactive notebook session (Jupyter, R Markdown) |
| `query` | Database query execution |
| `interactive` | Interactive analysis session |
| `container` | Pre-built container environment |
| `custom` | Other execution type (describe in execution context) |


#### Workflow Execution Context

For workflow execution, the execution context SHOULD be a `Dataset` that conforms to the [Workflow RO-Crate](https://w3id.org/workflowhub/workflow-ro-crate/1.0) profile. The identifier SHOULD be a permalink or versioned URL (e.g. [https://workflowhub.eu/workflows/289?version=1](https://workflowhub.eu/workflows/289?version=1)) or MAY be a nested directory within the BagIt payload directory (e.g. `"workflow289.1/"`).
...
#### Script/Code Execution Context

For single script or code file execution, the execution context SHOULD be a `SoftwareSourceCode` entity:

```json
{
  "@id": "#execution-context",
  "@type": "SoftwareSourceCode",
  "name": "Survival analysis script",
  "programmingLanguage": {"@id": "https://www.r-project.org/"},
  "codeRepository": "https://github.com/org/analysis-scripts",
  "version": "1.2.0",
  "author": [
...
#### Interactive Session / Notebook Execution Context

For notebook or interactive session execution, the execution context SHOULD use `SoftwareSourceCode` and/or `CreativeWork`:

```json
{
  "@id": "#execution-context",
  "@type": ["SoftwareSourceCode", "CreativeWork"],
  "conformsTo": {"@id": "https://jupyter.org/"},
  "name": "Exploratory data analysis session",
  "programmingLanguage": {"@id": "https://www.python.org/"},
  "author": {"@id": "https://orcid.org/0000-0001-1234-5678"},
  "dateCreated": "2026-01-15"
}
```


#### SQL Query Execution Context

For database query execution, the execution context SHOULD be a `SoftwareSourceCode` with a `programmingLanguage` indicating SQL:

```json
{
  "@id": "#execution-context",
  "@type": "SoftwareSourceCode",
  "programmingLanguage": {"@id": "#sql"},
  "name": "Cohort extraction query",
...
#### Container/Environment Execution Context

For pre-built container environment execution, the execution context SHOULD be a `SoftwareApplication`:

```json
{
  "@id": "#execution-context",
  "@type": "SoftwareApplication",
  "name": "Pre-approved analysis environment",
  "containerImage": "ghcr.io/tre/approved-env:v3.1",
...
#### Workflow Execution Context Example with Distribution
...
In the above example, the `mainEntity` points to the execution context `Dataset` that conforms to the Workflow RO-Crate Profile and references the ZIP download URI using `distribution`, therefore the client can download the workflow directly without needing to follow Signposting headers.
...
The CreateAction MUST reference the execution context using `instrument`.
...
```json
{
  ...
  "instrument": {"@id": "#execution-context"},
  "name": "Execute query 12389",
  "object": [
    {"@id": "input1.txt"}
  ]
}
...
The requesting agent SHOULD include `hasCredential` to record relevant qualifications and access agreements (see [User-Attribute Access Metadata](#user-attribute-access-metadata)).
...
```json
{
  ...
  "hasCredential": [
    {"@id": "#safe-researcher-training"},
    {"@id": "#tre-access-agreement"}
  ]
},
{
...
**Tip**: While the [FormalParameter](https://www.researchobject.org/ro-crate/1.1/workflows.html#describing-inputs-and-outputs) SHOULD match the definitions within the execution context (e.g. the Workflow Crate referenced from `mainEntity`), the only requirement from this profile is that their `name` is programmatically recognized by the execution engine for binding input parameters of the particular execution. 


### Outputs


If the execution has successfully completed, that is the `CreateAction` has `actionStatus` set to `http://schema.org/CompletedActionStatus`, the output data entities SHOULD be referenced from the `results` array.
...
**Tip**: Implementations may need to inspect the `FormalParameter` of the referenced execution context (e.g. Workflow Crate) to propagate a human readable _name_ and _encodingFormat_ file format of the inputs and output in this crate.
...
## User-Attribute Access Metadata

TRE operators require transparency around access control tied to user attributes such as training certifications and organisational agreements.

### User Qualification Recording

The requesting agent SHOULD document relevant credentials using `hasCredential`, referencing `EducationalOccupationalCredential` entities:
...
},
{
  ...
  "name": "Safe Researcher Training Certificate",
  "credentialCategory": "training",
  "dateAwarded": "2026-01-10",
  "validUntil": "2027-01-10",
  "recognizedBy": {"@id": "#uksa"}
},
{
  "@id": "#tre-access-agreement",
  "@type": "EducationalOccupationalCredential",
  "credentialCategory": "agreement",
  "name": "TRE72 Data Access Agreement",
  "dateAwarded": "2026-02-01"
}
```

### Dataset-Level Access Requirements

Datasets referenced in the crate MAY document their access requirements using `accessRequirements`:

```json
{
  "@id": "#nhs-cohort-dataset",
  "@type": "Dataset",
  "accessRequirements": {
    "@id": "#dataset-access-policy"
  }
},
{
  "@id": "#dataset-access-policy",
...
Data entities referenced as inputs or outputs SHOULD include characterisation metadata to clarify data types, synthetic status, and version information.
...
```json
{
  ...
  "variablesMeasured": [
    {"@id": "#age-variable"},
    {"@id": "#postcode-variable"}
  ]
}
```

The following RECOMMENDED properties extend data entity descriptions:

| Property | Type | Description |
|----------|------|-------------|
| `dataType` | Text or array | Classification of the data. Values include: `tabular`, `free-text`, `imaging`, `genomic`, `time-series` |
| `isSynthetic` | Boolean | Whether the data is synthetic rather than real |
| `syntheticGenerationMethod` | Reference | If synthetic, reference to the generation process |

### Column/Variable-Level Metadata

For tabular data, individual variables SHOULD be described using `variablesMeasured` referencing `PropertyValue` entities:
...
```json
{
  ...
  "@type": "PropertyValue",
  "name": "patient_age",
  "description": "Patient age at index date",
  "valueType": "integer",
  "measurementTechnique": "Calculated from DOB",
  "privacyAnnotation": "quasi-identifier"
}
```


## Software Authorship and Provenance

Research teams develop custom software and instruments that should be properly attributed. Key external software dependencies also need authorship tracking for citation and reproducibility.

### Software Author Attribution

All `SoftwareApplication` and `SoftwareSourceCode` entities SHOULD include authorship information:

```json
{
  "@id": "#analysis-tool-v2",
  "@type": "SoftwareApplication",
  "name": "Secure Computation Toolkit",
  "version": "1.2.3",
  "author": [
    {"@id": "https://orcid.org/0000-0001-1234-5678"},
    {"@id": "#research-software-team"}
  ],
  "contributor": [
    {"@id": "https://orcid.org/0000-0002-1234-5678"}
  ],
  "funder": {"@id": "https://gtr.ukri.org/projects?ref=10012345"},
  "codeRepository": "https://github.com/org/secure-toolkit",
  "license": {"@id": "https://spdx.org/licenses/Apache-2.0"}
},
{
  "@id": "#research-software-team",
  "@type": "Organization",
  "name": "TRE Research Software Engineering Team",
...
Support proper software citation following CITATION.cff / CodeMeta patterns:
...
```json
{
  ...
  "author": [
    {"@id": "#matthias-templ"},
    {"@id": "#bernhard-meindl"},
    {"@id": "#alexander-kowarik"},
    {"@id": "#johannes-gussenbauer"}
  ],
  "citation": {"@id": "#sdcmicro-paper"},
  "url": "https://cran.r-project.org/package=sdcMicro",
  "identifier": "https://doi.org/10.18637/jss.v067.i04"
},
{
  "@id": "#sdcmicro-paper",
  "@type": "ScholarlyArticle",
  "name": "sdcMicro: Statistical Disclosure Control Methods for Anonymization of Microdata and Risk Estimation",
  "author": [
    {"@id": "#matthias-templ"},
    {"@id": "#alexander-kowarik"},
    {"@id": "#bernhard-meindl"}
...
### External Key Software Attribution

For critical external dependencies that should be cited or attributed, use `isKeyDependency` to flag them:
...
},
{
  ...
  "name": "tidyverse",
  "version": "2.0.0",
  "author": {"@id": "#hadley-wickham"},
  "citation": {"@id": "#tidyverse-citation"},
...
},
{
  ...
  "@type": "Person",
  "name": "Hadley Wickham",
  "affiliation": {"@id": "https://ror.org/05qghxh33"}
}
```

### Software Properties Summary

| Property | Type | Description |
|----------|------|-------------|
| `author` | Person/Organisation | Creator(s) of the software |
| `contributor` | Person/Organisation | Other contributors |
| `funder` | Organisation/Grant | Funding source |
| `citation` | ScholarlyArticle | Preferred citation |
| `codeRepository` | URL | Source code location |
| `developmentStatus` | URL | Repostatus.org badge |
| `isKeyDependency` | Boolean | Flag for critical dependencies requiring citation |
| `isPartOf` | Project | Associated research project |


## Review process

The Five Safes RO-Crate may face several reviews both before and after execution, automated and manual. To record that such review will or has taken place, a series of additional [`Action`](http://schema.org/docs/actions.html) contextual entities SHOULD be related to the root data entity using `mentions`.
...
Assessment actions SHOULD indicate an [actionStatus](#execution-states) to reflect the outcome or pending nature of the assessment. Each assessment SHOULD have the root data entity (typically `{"@id": "./"}`) listed under `object`, and MAY include additional entities that were assessed.
...
A crate that has been validated according to RO-Crate specifications and this profile SHOULD _mention_ an _AssessAction_ which _instrument_ refers to the profile entity, and an `additionalType` referring to `https://w3id.org/shp#ValidationCheck`
...
{  
  ...
  "object": {"@id": "./"},
  "instrument": {"@id": "https://w3id.org/5s-crate/0.5"},
  "agent": {"@id": "#validator-a4a66c63-2fe0-4c57-830d-268a40718313"},
  "actionStatus": "http://schema.org/CompletedActionStatus"
}
```

The validation phase MAY perform any additional syntactic or semantic checks required by the particular TRE and execution context, e.g. correspondence between provided and expected input parameters, in which case this should be reflected by adding such entities to `object` (checked) and `instrument` (expected) as arrays.


#### Execution context retrieval phase

The referenced execution context (e.g. a workflow crate) may be retrieved by the TRE before Sign-off or Execution, potentially using a local proxy (see [Finding the RO-Crate archive](#finding-the-ro-crate-archive) and [Security considerations](#security-considerations)). In this case, the retrieval SHOULD be indicated by a [DownloadAction](http://schema.org/DownloadAction) with the execution context's distribution as `object` (indicating the URL that was downloaded from, potentially following Signposting). 

Implementations MAY choose to unpack and add the execution context folder to the Bagit, in which case it SHOULD be indicated as an additional data entity referenced as _result_, which reference the `mainEntity` from `sameAs` and the download location in `distribution`. 
...
Before executing a Five Safes RO-Crate, the TRE SHOULD check if the requesting agent is permitted to execute the particular computation on behalf of the responsible project. This SHOULD include checks against the Agreement policy data maintained by the TRE. This may be a manual and/or automated check, as indicated by `agent`. The `object` SHOULD additionally reference the execution context and responsible project (unless these were not part of the sign-off checks).

Review comments MAY be attached to sign-off actions using `comment` referencing `Comment` entities (see [Review Comments](#review-comments)).
...
},
{
  ...
  "text": "Approved. Please ensure output files are clearly labelled before egress.",
  "about": {"@id": "#signoff-3b741265-cfef-49ea-8138-a2fa149bf2f0"}
}
```
...
Assessment actions MAY include `comment` referencing one or more `Comment` entities to record human review commentary. Each `Comment` SHOULD include `author`, `dateCreated`, `text`, and `about` (referencing the assessment action to which the comment is attached).


##### Multi-stage Review Workflow

For complex approval processes, chained assessments MAY use `isBasedOn` to indicate dependencies between review stages:
...
In this phase, the approved execution is performed within the TRE. The `CreateAction` of the execution will update its `actionStatus` and acquire a `startTime` and `endTime`.

When the execution is in `http://schema.org/CompletedActionStatus` or `http://schema.org/FailedActionStatus`, the crate SHOULD also follow the [Provenance Crate](https://w3id.org/ro/wfrun/provenance/0.2) profile, e.g. the execution outputs data entities will be listed as `result`.
...
Note that even if the execution action is in `http://schema.org/CompletedActionStatus`, two additional phases are required to pass before the Five Safes RO-Crate can be considered "finished", see the following subsections.


#### Disclosure phase

Before execution results are returned from the TRE, a disclosure check SHOULD be performed, e.g. to verify the execution has not revealed sensitive data. Depending on the execution context and TRE data this may be automated and/or manual as indicated by `agent`.

##### Disclosure Check Details

The disclosure `AssessAction` SHOULD capture the type of check performed and the tools used. The following properties extend the `AssessAction` for the disclosure phase:

| Property | Type | Description |
|----------|------|-------------|
| `disclosureCheckType` | Text | One of: `"manual"`, `"automated"`, or `"hybrid"` |
| `primaryDisclosureRisk` | Reference | Assessment of direct identification risks |
| `secondaryDisclosureRisk` | Reference | Assessment of indirect/inferential risks |
| `approvedBy` | Person | Person with final decision authority for egress (distinct from `agent` performing the check) |
| `instrument` | SoftwareApplication | SHOULD reference the tool/methodology used for automated checking |

Example of a completed disclosure check:
...
```json
{
  ...
  "disclosureCheckType": "automated",
  "instrument": {"@id": "#sdcTool-v1.2"},
  "object": {"@id": "./"},
  "agent": {"@id": "https://orcid.org/0000-0001-1234-5678"},
  "approvedBy": {"@id": "https://orcid.org/0000-0002-1234-5678"},
  "actionStatus": "http://schema.org/CompletedActionStatus"
}
```

Example of a pending disclosure check:
...
When automated disclosure checking is used, the approval of the tool itself SHOULD be documented:

```json
{
  "@id": "#sdcTool-v2.1",
  "@type": "SoftwareApplication",
  "name": "Statistical Disclosure Control Tool",
  "version": "2.1",
  "approvalAction": {"@id": "#tool-approval-action"}
},
{
  "@id": "#tool-approval-action",
  "@type": "AssessAction",
  "name": "Approval of SDC Tool for output checking",
  "agent": {"@id": "#governance-committee"},
  "endTime": "2026-01-15",
  "actionStatus": "http://schema.org/CompletedActionStatus"
}
```

If a crate fails the disclosure phase, its content such as execution results MUST NOT be included in the returned crate returned to the user. Likewise, its execution `CreateAction` and corresponding output data entities SHOULD be removed from the metadata file.
...
The root data entity's `datePublished` SHOULD be updated to the time the manifest is last written. The `publisher` SHOULD be updated to reflect the executing TRE. The `mentions` MUST be expanded to include all the `AssessAction`s recorded by the TRE. `hasPart` MUST include (possibly through intermediate folders as `Dataset`) any `result` data entities now referred to from the execution's `CreateAction`. 

The _[licence](https://www.researchobject.org/ro-crate/1.1/contextual-entities.html#licensing-access-control-and-copyright)_ SHOULD be included to describe the licence of the execution output data, either an open licence such as Creative Commons, or a restrictive (typically TRE-specific) conditions of access.
...
## Enhanced Provenance Capture

### Data Transformation/Degradation Recording

To document intentional data modifications such as anonymisation, pseudonymisation, or aggregation, an `Action` with `additionalType` referencing `https://w3id.org/shp#DataTransformation` SHOULD be recorded:

```json
{
  "@id": "#transform-anonymisation",
  "@type": "Action",
  "additionalType": {"@id": "https://w3id.org/shp#DataTransformation"},
  "name": "K-anonymisation applied to demographic fields",
  "description": "Generalisation of age to 5-year bands, suppression of postcodes",
  "transformationType": "anonymisation",
  "object": {"@id": "#input-dataset"},
  "result": {"@id": "#transformed-dataset"},
  "instrument": {"@id": "#anonymisation-tool"},
  "agent": {"@id": "#tre-operator"}
}
```

The following transformation types are RECOMMENDED values for `transformationType`:
...
When recording execution provenance, the software environment SHOULD be documented using the following RECOMMENDED properties:

| Property | Type | Description |
|----------|------|-------------|
| `softwareRequirements` | Array of SoftwareApplication | Dependency software entities |
| `containerImage` | URI | URI to container image used |
| `runtimePlatform` | Text | Operating system/platform description |
...
```json
{
  ...
  "runtimePlatform": "Ubuntu 22.04 LTS",
  "containerImage": "ghcr.io/tre/analysis-env:v2.3"
},
{
  "@id": "#r-4.3.1",
  "@type": "SoftwareApplication",
...
### Mapping Decisions Documentation

For data harmonisation (e.g. OMOP mappings), an `Action` with `additionalType` referencing `https://w3id.org/shp#DataMapping` SHOULD be recorded:

```json
{
  "@id": "#omop-mapping",
  "@type": "Action",
  "additionalType": {"@id": "https://w3id.org/shp#DataMapping"},
  "name": "OMOP CDM mapping for diagnosis codes",
  "description": "ICD-10 codes mapped to OMOP concepts using OHDSI vocabulary v5.0",
  "object": {"@id": "#source-diagnosis-data"},
  "result": {"@id": "#omop-condition-occurrence"},
  "instrument": {"@id": "#ohdsi-vocabulary-v5"},
  "mappingAssumptions": "Unmapped codes assigned to concept_id 0; historical diagnoses included",
  "mappingCoverage": "94.3%"
}
```

<!-- ### Minimum Metadata per Stage

| Stage | Required Metadata |
|-------|-------------------|
| **Ingress** | Requesting agent, responsible project, execution context (workflow/script/etc.), inputs |
| **Workspace** | + Execution status, intermediate outputs, environment, software dependencies |
| **Egress** | + All assessments, disclosure checks, final outputs, provenance chain, software citations | -->
...
Allowing execution of any execution context (including Workflow Crates, scripts, notebooks, and queries) effectively allows execution of arbitrary code. It is RECOMMENDED to check against a list of pre-approved executions (see 
[Sign-off phase](#sign-off-phase)), e.g. using file checksums or cryptographic signatures. 

Clients parsing and unpacking ZIP files, JSON metadata, execution context definitions and BagIt manifests SHOULD apply reasonable security measures to limit the possibility of an attacker to consume excessive disk, CPU or memory resources, as well as escaping any file directory or execution container jails. For instance, clients should check for invalid file path characters, relative paths or symbolic links escaping the crate, as well guard against _zip bomb_ attacks.

Pre-approved executions could be exploited by a malicious attacker if they, the execution engine or their tools themselves have security vulnerabilities, e.g. by using hand-crafted input parameters that by-passes command line escapes. Executions are not guaranteed to complete in a given timescale; sufficient timeouts and resource usage restrictions SHOULD therefore be applied by the execution engine.
...
Malicious clients MAY attempt to reference URLs or IP addresses that are only accessible within a TRE. Implementers MUST perform any URL downloads (such as Workflow RO-Crates, script repositories, or container images) in a way that does not access the secured TRE network, e.g. from a _Demilitarized Zone_ (DMZ) with a network firewall restricting access to the TRE, or through a proxying repository controlled by TRE administrators.
...
The crate MAY include references (e.g. S3 URIs) to sensitive data, in which case the implementation and executed computation SHOULD protect against divulging sensitive information (directly or indirectly) in the _File_ identifiers, use UUID v5 hashing [[RFC4122](https://doi.org/10.17487/rfc4122)]  to hide sensitive identifiers.
...
1. **Generalised execution model:** The tight coupling to Workflow RO-Crate via `mainEntity` is relaxed. Whilst v0.4 crates remain valid (see below), new crates SHOULD use the execution context pattern.

### Backward Compatibility

1. **Workflow references:** Implementations MUST continue to accept v0.4 crates where `mainEntity` directly references a Workflow RO-Crate `Dataset`
2. **Implicit execution type:** Direct Workflow RO-Crate references SHOULD be treated as equivalent to `executionType: "workflow"`

### Additions

1. **New properties are RECOMMENDED**, not MUST (except where noted)
2. **Disclosure phase:** Existing `AssessAction` extended with new optional properties (`disclosureCheckType`, `approvedBy`, risk assessments)
3. **Software provenance:** `SoftwareApplication` and `SoftwareSourceCode` extended with authorship properties
4. **Execution types:** New `executionType` property and execution context entity pattern
5. **User-attribute access metadata:** `hasCredential` on requesting agents and `accessRequirements` on datasets
6. **Dataset characterisation:** `dataType`, `isSynthetic`, `variablesMeasured` properties
7. **Provenance capture:** Data transformation/degradation recording, software environment documentation, mapping decisions
8. **Review comments:** `comment` on assessment actions, multi-stage review workflow with `isBasedOn`

### Upgrade Path

To migrate a v0.4 crate to v0.5:
